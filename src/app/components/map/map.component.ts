...
export class MapComponent implements OnInit, OnDestroy, AfterViewInit {
  ...
  private destinationImageSeries2;
  private destinationImageTemplate2;
  private targetSVG =
  ...
  getDestinationCity(destinationCityForm: FormGroup) {
    if (this.destinationCity2.length >= 1) {
      this.subscription.add(
        this.service
          .updateCity(
            this.destinationCity2[this.destinationCity2.length - 1].id,
            destinationCityForm.value.destination
          )
          .subscribe((city) => {
            this.destinationCity2[this.destinationCity2.length - 1] = city;
            this.destinationImageSeries2.data = this.destinationCity2;
          })
      );
      this.subscription.add(
        this.service
          ...
          .subscribe((city) => {
            this.destinationCity2.push(city);
            this.destinationImageSeries2.data = this.destinationCity2;
          })
      );
    } else if (this.destinationCity.length === 1) {
      this.subscription.add(
        this.service
          .updateCity(
            this.destinationCity[this.destinationCity.length - 1].id,
            destinationCityForm.value.destination
          )
          .subscribe((city) => {
            this.destinationCity[this.destinationCity.length - 1] = city;
    ...
    if (this.destinationCity2.length >= 1) {
      changeLink.events.on('hit', () => {
        showLines(
          this.destinationImageSeries2.dataItems.getIndex(
            this.destinationImageSeries2.dataItems.length - 2
          )
        );
      });
    } else if (this.destinationCity.length === 1) {
      changeLink.events.on('hit', () => {
        showLines(this.destinationImageSeries.dataItems.getIndex(0));
      });
    } else if (this.destinationCity.length === 0) {
      changeLink.events.on('hit', () => {
  ...
  ngAfterViewInit() {
    this.zone.runOutsideAngular(() => {
      ...
      usaTemplate.fill = am4core.color('#313131');
      ...
      canadaTemplate.fill = am4core.color('#313131');
      canadaTemplate.fillOpacity = 0.6;
      canadaTemplate.events.on('hit', (ev) => {
        ev.target.series.chart.zoomToMapObject(ev.target);
      });

      const hs = polygonTemplate.states.create('hover');
      hs.properties.fill = am4core.color('#7b1fa2');
      const hus = usaTemplate.states.create('hover');
      hus.properties.fill = am4core.color('#7b1fa2');
      const hcan = canadaTemplate.states.create('hover');
      hcan.properties.fill = am4core.color('#7b1fa2');
      ...
      // destination series after first input (small targets)
      this.destinationImageSeries2 = this.chart.series.push(
        new am4maps.MapImageSeries()
      );
      this.destinationImageTemplate2 = this.destinationImageSeries2.mapImages.template;

      this.destinationImageTemplate2.nonScaling = true;
      this.destinationImageTemplate2.tooltipText = '{title}';
      this.destinationImageTemplate2.fill = interfaceColors.getFor(
        'alternativeBackground'
      );
      this.destinationImageTemplate2.setStateOnChildren = true;
      this.destinationImageTemplate2.states.create('hover');

      this.destinationImageTemplate2.propertyFields.latitude = 'latitude';
      this.destinationImageTemplate2.propertyFields.longitude = 'longitude';
      this.destinationImageTemplate2.propertyFields.id = 'id';

      const destinationHitCircle2 = this.destinationImageTemplate2.createChild(
        am4core.Circle
      );
      destinationHitCircle2.radius = 7;
      destinationHitCircle2.fillOpacity = 1;
      destinationHitCircle2.fill = interfaceColors.getFor('background');

      const destinationTargetIcon2 = this.destinationImageTemplate2.createChild(
        am4core.Sprite
      );
      destinationTargetIcon2.scale = 0.7;
      destinationTargetIcon2.path = this.targetSVG;
      destinationTargetIcon2.horizontalCenter = 'middle';
      destinationTargetIcon2.verticalCenter = 'middle';
    });
  }
